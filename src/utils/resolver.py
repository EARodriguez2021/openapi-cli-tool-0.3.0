...
def _find_reference(link, base_file_path, file_control):
    ...
    spec_file = base_file_path if file_path == '' else os.path.join(
        os.path.dirname(base_file_path), file_path)
    try:
        ...
        print('Failed to load reference. file=' + base_file_path +
              ' $ref=' + link + ' (' + str(e) + ')')
        return '', spec_file
...
def resolver(file_path, data, file_control=FileControl()):
    ...
    for key, value in data.items():
        if key == '$ref':
            new_value, base_file_path = _find_reference(
                value, file_path, file_control)
...
def resolve_once(file_path, data, file_control=FileControl()):
    if not isinstance(data, dict):
        return data
    if '$ref' in data:
        new_value, _ = _find_reference(
            data['$ref'], file_path, file_control)
        return new_value
    return data
