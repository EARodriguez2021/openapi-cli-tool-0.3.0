...
def run_bundle(spec_paths, filename=None):
    ...
    if collection.len() == 0 and filename is None:
        ...
        header_file = spec_paths[0] if spec_paths[0] != collection.get()[
            0].file else collection.get()[0].file
    ...
    for route in collection.get():
        ...
        resolved_spec = resolver(
            route.file, route.spec, repository.file_control)
